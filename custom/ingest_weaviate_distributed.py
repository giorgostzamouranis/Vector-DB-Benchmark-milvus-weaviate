...
def recreate_class(client, class_name: str):
    ...
    # Delete the class if it already exists (Weaviate v4 collections API).
    if client.collections.exists(class_name):
        client.collections.delete(class_name)

    # Create the class/collection 
    # - vectorizer: none (we provide vectors)
    # - vector index: HNSW tuned with the provided parameters
    # - properties: store an integer doc_id to map results back to the dataset ids
    client.collections.create(
        ...
        # Sharding config (desired_count = 1 for single-node / baseline)
        sharding_config=Configure.sharding(
                        desired_count=2,             
                        virtual_per_physical=128,
                        desired_virtual_count=128,
    ),
        # Replication config (fault tolerance; factor=1 means no replication).
...
def main():
    ...
    # Derive a Weaviate class name from dataset + amount, normalized to start with uppercase.
